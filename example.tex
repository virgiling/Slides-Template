...
% Verbatim content requires the standard Beamer `fragile` frame option.
\begin{frame}[fragile]{Code and Monospace Text}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.59\textwidth}
      \begin{block}{Plain verbatim}
\begin{verbatim}
...
\end{verbatim}
    ...
    \begin{column}{0.36\textwidth}
      MD IO is applied automatically to:
      \begin{itemize}
        \item \texttt{inline code}
        \item \texttt{verbatim} blocks
        \item file names and commands
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{Links, Footnotes, and Navigation}
  ...
  \begin{itemize}
    ...
    \item Manual citation: the class builds on Beamer's presentation model~\cite{beamer}.
...
% For a long bibliography, add the `allowframebreaks` frame option.
